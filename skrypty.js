...
========================================================================
*/
function wstaw_tag_dla_interakcji_z_okienkiem(znacznik_do_dopisania, url) {
    if (znacznik_do_dopisania.indexOf("a") != '-1') {
        //Wywołanie ustawień						
        zmodyfikowany_zaznaczony_tekst = "<" + znacznik_do_dopisania + " href='" + url + "' style='" + zebrane_style_z_checkboxow + "'>" + text_w_zmiennej + '</' + znacznik_do_dopisania + '>';
    }
    if (znacznik_do_dopisania.indexOf("img") != '-1') {
        //Wywołanie ustawień						
        zmodyfikowany_zaznaczony_tekst = "<" + znacznik_do_dopisania + " alt='" + url + "' src='" + text_w_zmiennej + "' style='" + zebrane_style_z_checkboxow + "'/>";
    }
    //Sklejanie całości		
    zwracany_tekst_po_sklejeniu = sklej_zmodyfikowany_tekst(zmodyfikowany_zaznaczony_tekst);
...
function pobierz_zawartosc_zajawek()
{
    ...
    for (var z = 0; z < cala_zawartosc_panelu_zajawek.length; z++)
    {   //Odwołaj się do jednego Diva - jedna zajawka
        var pojedynczy_div_zajawki = cala_zawartosc_panelu_zajawek[z].children;

        for (var c = 0; c < pojedynczy_div_zajawki.length; c++)
        {   //Odwołaj się do jednego inputa w danej zajawce
...
function wyswietl_zawartosc_zajawek() {
    ...
    for (var z = 0; z < id_inputow_zajawek_do_tablicy_zawartosci.length; z++)
    {
        ...
        for (var v = 0; v < id_inputow_zajawek_do_tablicy_zawartosci.length / 3; v++)
        {
            ...
            for (var q = 0; q < 3; q++)
            {
                ...
                {
                    element_zajawki = '<a href="' + value_inputow_zajawek_do_tablicy_zawartosci[3] + '">' + value_inputow_zajawek_do_tablicy_zawartosci[table_filler] + '</a>\n';
...
/* 
=============================================
= USUWANIE WSZYTKICH ZNACZNIKÓW I FORMATU   =
=============================================
*/
...
function dopisz_formularz(id_buttona) {
    ...
    input_formularza_1.id = "pierwszy_input";
    var input_formularza_2 = document.createElement("INPUT");
    input_formularza_2.id = "drugi_input";
...
function zatwierdz_button_formularza() {
    var odwolanie_do_formularza = document.getElementById('formularz').firstElementChild;
    ...
    element_z_ktorego_usuwam.removeChild(element_do_usuniecia);
    //----------------Usuwanie tak jak dla X button----------------//
    zawartosc_formularza_w_okienku = odwolanie_do_formularza.value;
    wstaw_tag_dla_interakcji_z_okienkiem(znacznik_do_dopisania, zawartosc_formularza_w_okienku);
}
