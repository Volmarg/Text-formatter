(function (window) {
	window.jsColorPicker = function(selectors, config) {
		var renderCallback = function(colors, mode) {
				...
				/* Moje dodatkowe funkcje generujące podgląd */
				
				colorTlaNaglowkaZajawki(); 
				colorTekstuNaglowkaZajawki();
				colorTlaTekstuZajawki();
				colorTekstuZajawki();
				colorObamowaniaZajawki();
				colorWnetrzaZajawki();
		...
		for (var n = 0, m = elms.length; n < m; n++) {
			...
			if (config === 'destroy') {
				...
				elm.setAttribute('data-colorMode','rgb');
